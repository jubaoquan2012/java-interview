...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        /** Exclusive（独占，只有一个线程能执行，如ReentrantLock）空节点:EXCLUSIVE = null */
    ...
    private Node addWaiter(Node mode) {
        ...
        //执行到这里,尾结点为null ===>说明队列为空,
        //当前线程是第一个加入同步队列进行等待的线程
        enq(node);
        return node;
    }
    ...
    private void doReleaseShared() {
        /*
         * 下面的循环在 head 节点存在,后继节点的情况下，做了两件事情：
    ...
    private void setHeadAndPropagate(Node node, int propagate) {
         ...
         * 条件 propagate > 0 判断是否唤醒后继节点是不充分的
         */
        if (propagate > 0 || h == null || h.waitStatus < 0 ||
             ...
             * 至于 s == null :新增节点,pred.next 还没有链接上
             */
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // pre结点的状态大于0，也就是CANCELLED 状态
        if (ws > 0) {
            do {
                // 向队列的前驱节点找,一直到找到一个状态码不小于0的,也就是非CANCEL状态的
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            pred.next = node;
        } else {// 如果前驱节点的状态是小于0的， ws must be 0 or PROPAGATE.
            //CAS方式将前驱节点的状态更新为SIGNAL。同时不对线程阻塞
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                /**第一个if分句*/
                //该节点的前驱节点是头节点，就尝试去获取一下许可,当获取许可成功后，
                // 就可以将头节点设置为当前的节点,并将后继节点置空，重置状态
                if (p == head && tryAcquire(arg)) {
                    //将当前结点设置为队列头结点, 并且
                    // head = node; node.thread = null;
                    // node.prev = null
                    setHead(node);
                    // help GC
                    p.next = null;
                ...
                /**第二个if分句*/
                //当前节点node的前驱节点p 的waitStatus = SIGNAL 状态才会返回true,
                // 然后执行parkAndCheckInterrupt ,其他状态都是返回false, 不进行阻塞
