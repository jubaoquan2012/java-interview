...
public class InputOutputStreamDemo {
     ...
     *      这三句代码的功 能是一样的，其中第三句最为便捷。
     *          注意：一旦 要指定其他编码时，绝对不能用子类，必须使用字符转换流。什么时候用子类呢？
     *              条件：
     *                  1、 操作的是文件。2、使用默认编码。
     *      总结：
     *          字 节--->编码表--->字符 ： 看不懂的--->看的懂的。  需要读。输入流。 InputStreamReader
     *          字符 --->编码表--->字节 ： 看的懂的--->看不懂的。  需要写。输出流。 OutputStreamWriter
     *  ------------ ----------------------------------------------------------------------------------------
     * 上面程序在读取含有 中文的文件时，我们并没有看到具体的中文，而是看到一些数字，这是什么原因呢？
     * 既然看不到中文，那么 我们如何对其中的中文做处理呢？要解决这个问题，我们必须研究下字符的编码过程。
     * 引出字符编码：
     *
     *   字符编码表
     *
